...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const { email, password, firstname, lastname, ...rest } = body;
        ...
        const response = await fetch(apis.registerV1UrlIP, {
            method: "POST",
            headers: {
        ...
        const nextResponse = NextResponse.json(
            { message: "Registration successful", data: data.data || data },
            { status: 200 }
        );
        ...
        logger.error(err, session, `api/register`);
